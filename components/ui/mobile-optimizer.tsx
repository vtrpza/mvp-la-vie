...
export function MobileOptimizer({ children }: MobileOptimizerProps) {
  ...
  useEffect(() => {
    ...
    if (mobile) {
      ...
      mobileCSS.href = '/mobile-optimized.css'
      mobileCSS.media = 'screen and (max-width: 768px)'
      document.head.appendChild(mobileCSS)

      // Disable complex animations if preferred
      if (reducedMotion) {
      ...
      const noZoom = document.createElement('style')
      noZoom.textContent = `
      ...
      `
      document.head.appendChild(noZoom)
    }
  ...
  return (
    <div 
      className={`mobile-optimized ${isMobile ? 'mobile-device' : ''}`}
